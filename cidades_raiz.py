...
def cidades_raiz(numero_arquivo: int):
    ...
    df["ano_mais_quente"] = df['Lon']
    df['temp_mais_alta'] = df['Lon']
    df["ano_mais_frio"] = df['Lon']
    df['temp_mais_baixa'] = df['Lon']
    ...
    for index, row in df.iterrows():
        ...
        max_index = df_cidade['tmax'].idxmax()
        df.loc[index, 'ano_mais_quente'] = df_cidade['Date'][max_index]
        df.loc[index, 'temp_mais_alta'] = df_cidade['tmax'][max_index]

        # df['ano_mais_quente'][index] = df_cidade['Date'][max_index]
        # df['temp_mais_alta'][index] = df_cidade['tmax'][max_index]
        min_index = df_cidade['tmin'].idxmin()
        df.loc[index, 'ano_mais_frio'] = df_cidade['Date'][min_index]
        df.loc[index, 'temp_mais_baixa'] = df_cidade['tmin'][min_index]

        # df['ano_mais_frio'][index] = df_cidade['Date'][min_index]
        # df['temp_mais_baixa'][index] = df_cidade['tmin'][min_index]
    ...
    df.to_csv(f'saidas/cidades_temp_result/out_per_lat{numero_arquivo}.csv', index=False)
