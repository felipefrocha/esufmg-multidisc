...
def consolidate_datasus():
    with open(file=f'./{os.environ.get("NAME_FILE")}{randint(0,10000)}.csv', mode='a', newline='') as csvfile:
        spamwriter = csv.writer(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        rows = ['{}{}{}{}'.format(
            lista_1[randint(0, 29)],
            lista_2[randint(0, 29)],
            lista_3[randint(0, 29)],
            lista_4[randint(0, 29)]) for i in range(int(os.environ.get("INTERACTIONS")))]
        [spamwriter.writerow(row) for row in rows]
